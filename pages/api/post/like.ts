      ...
      if (isLiking) {
        try {
          ...
          if(userPostId?.userId !== userLiking?.userId){
            ...
            await Notification.updateOne({userId: userPostId?.userId}, { hasNotification: true, $push: {notifications: {userId: userLiking?.userId, message: 'curtiu sua publicação', redirect: `/user/posts/${postId}?q=${likeId}`, notificationId: likeId, itemToRemoveId: postId} } }  )
